...
function initLoc() {
    navigator.geolocation.getCurrentPosition(position => readyPage(position.coords.latitude, position.coords.longitude));
}

var smata;

function getPosWeather() {
    readyPage(
        document.getElementById('latenter').value,
        document.getElementById('longenter').value
    )
}

var durl;

function readyPage(latitude, longitude) {
    durl = `https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query=${latitude},${longitude}&forecast_days=5`; //Place your weatherstack API Call Here - access_key to be used: 5bc82451636190abd9d7afe6fe9b20b5

    $.ajax({url: durl, dataType: "jsonp"}).then(function (data) {
        smata = data;
        /*
        ...
        */
        ...
        document.getElementById("local_time").innerHTML = Date(data.location.localtime)
